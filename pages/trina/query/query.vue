<template>
	<view class="content" :style="{ height: autoHeight }">
		<view class="flex_x" style="margin-top: 20.833rpx;">
			<u-subsection :current="1" :list="['因公出行', '私人旅游']" class="btn" style="border-radius: 41.667rpx; width: 312.5rpx;"></u-subsection>
		...
		<view class="cardBackgroud">
          ...
          searchData.arrCity
			...
			<view class="card1">
				...
				<u-checkbox-group>
					<u-checkbox @change="checkboxChange" v-model="isCheck" :name="isCheck"><text style="color: #000000;">只看高铁动车</text></u-checkbox>
				</u-checkbox-group>
			...
			<view class="card1">
				<u-button type="warning" @click="toSerch" class="search" style="background: linear-gradient(to right, #ff9536, #ff7843);">开始搜索</u-button>
			</view>
		</view>
		<!-- 时间选择控件 -->
...
<script>
	...
	export default {
		data() {
			return {
				title: "Hello",
				src: "/static/imgs/train_exchange.png",
				show: false,
				mode: "date",
				maxDate: "2020-9-20",
		...
		onShow() {
			...
			// 非app平台页面高度
			// #ifndef APP-PLUS
			this.autoHeight = res.windowHeight + "px";
			// #endif
