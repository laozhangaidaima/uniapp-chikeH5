<template>
	<view class="content">

		<view class="flex_x ">
			<u-subsection vibrateShort :current="1" :list="['因公出行', '私人旅游']" class="btn" style="border-radius: 20px;width: 150px; "></u-subsection>
			<text class="btn2">差旅标准</text>
		</view>


		<view class="cardBackgroud">
			<view class="flex_x underLine mt30 pb30">
				<u-button class="btn3" @click="toCityPick">出发城市</u-button>
				<u-image width="31px" height="31px" :src="src" style="margin-top: 6.25rpx; "></u-image>
				<u-button class="btn3" @click="toCityPick">到达城市</u-button>
			</view>

			<view class="mt30 mb30 ">
				<u-calendar v-model="show" :mode="mode" :max-date="maxDate"></u-calendar>
				<u-button @click="show = true" class="startData">出发日期</u-button>
				<view style="display: block;height: 1px;width: 100%;background-color: #dcdfe6;" class="mt30">
				</view>
			</view>

			<view class="mb30 " style="width: 90%;margin: 41.66rpx auto; ">
				<u-button type="primary" @click="toSerch">搜索</u-button>
			</view>

		</view>


	</view>
</template>

<script>
	export default {
		data() {
			return {
				title: 'Hello',
				src: "data:image/png;base64,",
				show: false,
				mode: 'date',
				maxDate: "2020-9-20",

			}
		},
		onLoad() {

		},
		methods: {
			toCityPick() {
				uni.navigateTo({
					url: '/pages/cityPick/cityPick'
				});
			},
			toSerch(){
				uni.navigateTo({
					url: '/pages/airserch/airserch'
				});
			},
		}
	}
</script>

<style lang="scss" scoped>
	page {
		height: 100%;
		background-image: url('/static/airbg.jpg');
		background-size: cover;
		// margin-top: 20px;
	}


	.flex_x {
		display: flex;
	}

	.mt30 {
		margin-top: 62.5rpx;
	}

	.mt20 {
		margin-top: 41.66rpx;
	}

	.mb30 {
		margin-bottom: 62.5rpx;
	}

	.pb30 {
		padding-bottom: 62.5rpx;
	}

	.underLine {
		border-bottom: 0.5px solid #e5e5e5;
	}


	.content {

		height: 100%;
		display: flex;
		flex-direction: column;
		align-items: center;
		padding-left: 40rpx;
		padding-right: 40rpx;

		.btn {
			border-radius: 41.66rpx;
		}

		.btn2 {
			width: 145.83rpx;
			height: 72.91rpx;
			line-height: 72.91rpx;
			text-align: center;
			color: #ffffff;
			background: rgba(0, 0, 0, 0.5);
			border-radius: 41.66rpx;
			position: absolute;
			right: 20px;
		}


		.cardBackgroud {
			width: 100%;
			margin-top: 41.66rpx;
			border-radius: 8.33rpx;
			border: 2.08rpx solid #ebeef5;
			background-color: #fff;
			overflow: hidden;
			color: #303133;
			transition: .3s;

			.btn3 {
				font-weight: bold;
				font-size: 24px;
				font-weight: bold;
				font-family: -apple-system-font, "Helvetica Neue", sans-serif;

			}

			.startData {
				justify-content: left;
				margin-left: 20.83rpx;
				font-size: 17px;
			}
		}
	}

	// 按钮边框去除
	.u-hairline-border[data-v-6e15e680]:after {
		content: ' ';
		position: absolute;
		pointer-events: none;
		box-sizing: border-box;
		-webkit-transform-origin: 0 0;
		transform-origin: 0 0;
		left: 0;
		top: 0;
		width: 199.8%;
		height: 199.7%;
		-webkit-transform: scale(0.5, 0.5);
		transform: scale(0.5, 0.5);
		border: none;
		z-index: 1;
	}
</style>
